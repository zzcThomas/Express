...
<div class="row wrapper border-bottom white-bg page-heading">
 ...
 <c:if test="${detail.packageIsCrossBorder eq false}">
<div class="wrapper wrapper-content delivery">
		...
		<div class="col-sm-6 col-sm-offset-0 col-md-4 col-md-offset-1">
			<div class="ibox">
				...
				<div class="ibox-content info-card"  id="recv_info">
					...
					<div class="row">
						<div class="col-sm-12">
	                    	<input type="text" placeholder="收件人姓名" class="form-control m-b" id="recv_name" name="recv_name" value="${detail.recvName }"/>
	                    </div>
	                    <div class="col-sm-12">
	                    	<input type="text" placeholder="收件人手机号" class="form-control m-b" id="recv_phonenum" name="recv_phonenum" value="${detail.recvPhonenum }">
	                    </div>
	                    <div class="col-sm-6">
	                    	<select class="form-control m-b province" id="recv_province" name="recv_province" data-first-value=""  data-first-title="省份/自治区" data-value="${detail.recvProvince.id }">
                            </select>
	                    </div>
	                    <div class="col-sm-6">
	                    	<select class="form-control m-b city" id="recv_city"name="recv_city"  data-first-value=""  data-first-title="城市/地区" data-value="${detail.recvCity.id }">
                            </select>
	                    </div>
	                    <div class="col-sm-12">
	                    	<select class="form-control m-b area" id="recv_district" name="recv_district" data-first-value=""  data-first-title="区/县" data-value="${detail.recvDistrict.id }">
                            </select>
	                    </div>
	                    <div class="col-sm-12">
	                    	<textarea placeholder="详细地址" class="form-control m-b" id="recv_address" name="recv_address">${detail.recvAddress}</textarea>
	                    </div>
						<div class="col-sm-12">
	                    	<input type="text" placeholder="邮政编码" class="form-control" id="recv_code" name="recv_code" value="${detail.recvCode }"/>
	                    </div>
                	</div>
                	<div class="hr-line-dashed"></div>
                	<div class="row">
		...
		<div class="col-sm-6 col-sm-offset-0 col-md-4 col-md-offset-1">
			<div class="ibox">
				...
				<div class="ibox-content info-card"  id="send_info">
					...
					<div class="row">
						<div class="col-sm-12">
	                    	<input type="text" placeholder="寄件人姓名" class="form-control m-b" id="send_name" name="send_name" value="${detail.sendName }">
	                    </div>
	                    <div class="col-sm-12">
	                    	<input type="text" placeholder="寄件人手机号" class="form-control m-b" id="send_phonenum" name="send_phonenum" value="${detail.sendPhonenum }">
	                    </div>
	                    <div class="col-sm-6">
	                    	<select class="form-control m-b province" id="send_province" name="send_province" data-first-value=""  data-first-title="省份/自治区" data-value="${detail.sendProvince.id }">
                            </select>
	                    </div>
	                    <div class="col-sm-6">
	                    	<select class="form-control m-b city" id="send_city"name="send_city"  data-first-value=""  data-first-title="城市/地区" data-value="${detail.sendCity.id }">
                            </select>
	                    </div>
	                    <div class="col-sm-12">
	                    	<select class="form-control m-b area" id="send_district" name="send_district" data-first-value=""  data-first-title="区/县" data-value="${detail.sendDistrict.id }">
                            </select>
	                    </div>
	                    <div class="col-sm-12">
	                    	<textarea placeholder="详细地址" class="form-control m-b" id="send_address" name="send_address">${detail.sendAddress }</textarea>
	                    </div>
						<div class="col-sm-12">
	                    	<input type="text" placeholder="邮政编码" class="form-control" id="send_code" name="send_code" value="${detail.sendCode }">
	                    </div>
                	</div>
                	<div class="hr-line-dashed"></div>
                	<div class="row">
				...
				<div class="ibox-content">
					<div class="row">
						...
						<div class="col-sm-4 col-md-2">
							...
							<label><input type="radio" value="1" name="package_type" id="package_type" <c:if test="${detail.packageType eq 1 }">checked=""</c:if>> <i></i>文件</label>
							</div>
						</div>
						<div class="col-sm-4 col-md-2">
							<div class="radio i-checks">
							<label><input type="radio" value="2" name="package_type"<c:if test="${detail.packageType eq 2 }">checked=""</c:if>> <i></i>物件</label>
							</div>
						</div>
					</div>
					<div class="row">
						...
						<div class="col-sm-3">
							 <input type="text" class="form-control" id="package_inner_content" name="package_inner_content" value="${detail.packageInnerContent }"/> 
						</div> 
					</div>
					<div class="row">
                        <label class="col-sm-2 control-label">数量</label>
						<div class="col-sm-3">
							<input type="text" class="form-control" id="package_count" name="package_count" value="${detail.packageCount }">
						</div> 
					</div>
					<div class="row">
						...
						<div class="col-sm-4 col-md-2">
							<div class="radio i-checks">
							<label><input type="radio" value="true" name="package_is_price_protect" id="package_is_price_protect" <c:if test="${detail.packageIsPriceProtect eq true }">checked=""</c:if> > <i></i>是  </label>
							
							</div>
						</div>						
						<div class="col-sm-4 col-md-2">
							<div class="radio i-checks">
							<label><input type="radio" value="false" name="package_is_price_protect" <c:if test="${detail.packageIsPriceProtect eq false }">checked=""</c:if>  > <i></i>否</label>
							</div>
						</div>
					</div>
					<div class="row">
                        <label class="col-sm-2 control-label">保价价值(元)<br/><small class="text-navy">(选保价必填)</small></label>
						<div class="col-sm-3">
							 <input type="text" class="form-control" id="package_protect_price" name="package_protect_price" value="${detail.packageProtectPrice/100 }"/> 
						</div> 
					</div>
					<div class="row">
                        <label class="col-sm-2 control-label">保价理由<br/><small class="text-navy">(选保价必填)</small></label>
						<div class="col-sm-3">
							 <input type="text" class="form-control" id="package_protect_price_reason" name="package_protect_price_reason" value="${detail.packageProtectPriceReason }"/> 
						</div> 
					</div>
					<div class="row">
                        <label class="col-sm-2 control-label">对公快递事由<br/><small class="text-navy">(审批用)</small></label>
						<div class="col-sm-6">
							<textarea class="form-control"  id="inspection_apply_cause" name="inspection_apply_cause">${detail.inspectionApplyCause }</textarea>
						</div> 
					</div>
				</div>			
			</div>
					...
					<div class="row">
						...
						<c:forEach var="item" items="${packageExpressList}">
   						<div class="col-sm-3 col-md-2">
							<div class="radio i-checks">
							<label><input type="radio" value="${item.id}" name="package_expect_express" <c:if test="${detail.packageExpectExpress.id eq item.id }">checked=""</c:if> <c:if test="${item.id eq 0 }">id="package_expect_express"</c:if> /> <i></i>${item.name}</label>
							</div>
						</div>
						</c:forEach>
					</div>
	...
	<div class="row">
		<div class="col-sm-3 col-sm-offset-3">
			<a class="btn btn-primary btn-lg btn-submit" id="submit_btn"><i class="fa fa-check"></i>&nbsp;重新提交申请</a>
		</div>
		<div class="col-sm-2">
...
var layer_markinfo_index;
        $(document).ready(function () {
            ...
            $('#submit_btn').click(function(){
            	var check_message = {
            			...
            			"recv_province":["请选择收件人省份/直辖市",/^[0-9]+$/,"省份信息出错"], //数字不少于1位
            			"recv_city":["请选择收件人城市/区",/^[0-9]+$/,"城市区域信息出错"],//数字不少于1位
            			"recv_district":["请选择收件人区/县",/^[0-9]+$/,"区县信息出错"],//数字不少于1位
                        ...
                        "send_province":["请选择寄件人省份/直辖市",/^[0-9]+$/,"省份信息出错"],
                        "send_city":["请选择寄件人城市/区",/^[0-9]+$/,"城市区域信息出错"],
                        "send_district":["请选择寄件人区/县",/^[0-9]+$/,"区县信息出错"],
            ...
            $.each(['recv','send'],function(i,val){
	    		...
	    		$("#save_"+val+"_info_btn").click(function(){
	    			var check_message = {
	            			...
	            			"province":["请选择份/直辖市",/^[0-9]+$/,"省份信息出错"], //数字不少于1位
	            			"city":["请选择城市/区",/^[0-9]+$/,"城市区域信息出错"],//数字不少于1位
	            			"district":["请选择区/县",/^[0-9]+$/,"区县信息出错"],//数字不少于1位
    							...
    							if(info!=null){
    								try{
    									...
    									if(info.province!=null){
   										$('#'+rs+'_'+'province').find("option[value='"+info.province.id+"']").first().attr("selected",true);
   										$('#'+rs+'_'+'province').trigger('change');	
    									}
    									if(info.city!=null){
    									$('#'+rs+'_'+'city').find("option[value='"+info.city.id+"']").first().attr("selected",true);
    									$('#'+rs+'_'+'city').trigger('change');
    									}
    									if(info.district!=null){
	    									$('#'+rs+'_'+'district').find("option[value='"+info.district.id+"']").first().attr("selected",true);
	    									$('#'+rs+'_'+'district').trigger('change');
    									}
 ...
 <c:if test="${detail.packageIsCrossBorder ne false}"> 该页面无法修改跨境对公快递 </c:if>
</div>
